...
import { getLibraryGenerations, deleteCaptionAction, clearGenerationsAction } from '../actions';
...
export default function Library() {
  ...
  const loadGenerations = async () => {
    try {
      ...
      const data = await getLibraryGenerations(idToken);
      setGenerations(data);
    } catch (e) {
      console.error(e);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const idToken = await user!.getIdToken();
      await deleteCaptionAction(idToken, id);
  ...
  const clearHistory = async () => {
    ...
    try {
      setLoading(true);
      const idToken = await user!.getIdToken();
      await clearGenerationsAction(idToken);
      setGenerations([]);
  ...
  return (
    <div className="w-[96%] mx-auto min-h-[90vh] flex flex-col items-center animate-bg-gradient relative overflow-hidden rounded-[2.5rem] mt-4 mb-4 border border-white/20 shadow-2xl">
      ...
      <div className="w-[80%] mx-auto p-4 pt-32 pb-16 relative z-10 flex-1">
        ...
        <div className="mb-8 p-4 bg-white/90 backdrop-blur-sm rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-3 h-4 w-4 text-slate-400" />
            <input 
              type="text"
              placeholder="Search descriptions..." 
              className="w-full pl-9 p-2.5 bg-slate-50/50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
            />
          </div>
          <Select value={platformFilter} onValueChange={setPlatformFilter}>
            <SelectTrigger className="w-[180px] p-2.5 bg-slate-50/50 border border-slate-200 rounded-lg text-sm outline-none">
        ...
        <div className="space-y-6">
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-slate-500 bg-white/90 backdrop-blur-sm rounded-2xl border border-dashed border-slate-300">
              No saved captions found. Generate some magic first!
            </div>
          ) : (
            filtered.map(item => (
              <div key={item.id} className="bg-white/90 backdrop-blur-sm rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-all">
                ...
                <div className="p-6">
                    ...
                    {['direct', 'story', 'cta'].map((type, idx) => (
                      <TabsContent key={type} value={type} className="mt-0">
                        <div className="relative group">
                          <p className="whitespace-pre-wrap text-sm text-slate-700 bg-slate-50/50 border border-slate-100 rounded-xl p-5 leading-relaxed pr-12 min-h-[100px]">
                            {item.variants[idx]}
                          </p>
                          <button 
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    {item.hashtags.map((tag: string, i: number) => (
                      <span 
                        key={i} 
                        className="text-xs font-semibold text-slate-500 bg-white border border-slate-100 px-3 py-1.5 rounded-full cursor-pointer hover:border-indigo-200 hover:text-indigo-600 transition-colors"
